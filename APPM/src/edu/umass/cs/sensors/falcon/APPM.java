...
import edu.umass.cs.sensors.falcon.data.EventInfo;
import edu.umass.cs.sensors.falcon.model.PPM;
import edu.umass.cs.sensors.falcon.model.TemporalModel;
...
public class APPM {
	...
	/**
	 * Example demonstrating how to use APPM. Uses randomly generated sequence of events.
	 * @param args none
	 */
	//public static void main(String args[]){
		...
		ReadCVS fileReader = new ReadCVS("./res/appusage.csv");
		Result result = fileReader.run();
		//System.out.println(result.getNames().size()+" "+result.getTPs().size());
		...
		for(int i=0;i<100;i++) {
			...
			String event = "Event"+random.nextInt(5); // random event
			long currentTime = startTime+random.nextInt(8)*10000; //random time
			...
			double likelihood[] = appm.getEventLikelihoodInDeltaTime(currentTime, deltaTime, 0, 0, targetEvent);
			for(int j=0;j<likelihood.length;j++)
				System.out.println("Likelihood["+targetEvent[j]+"]="+likelihood[j]);
	...
	public static void main(String args[]){
		...
		for(int TOPK=1;TOPK<=5;TOPK++){
			...
			//Events for which temporal model is needed
			LinkedList<String> eventMonitorList = new LinkedList<String>();
			eventMonitorList.add(result.getNames().get(1));
			String predictions[] = null;
			//Register events for temporal model
			appm.registerEventsForTemporalModeling(eventMonitorList);
			
			long startTime = Long.valueOf(result.getTPs().get(1)); 
			int countOfPrediction=0;
			int countOfCorrectPrediction=0;
			for(int i=2;i<result.getNames().size();i++) {
				//Generate next event and next time
				String event = result.getNames().get(i); // next event
				long currentTime = startTime+Long.valueOf(result.getTPs().get(i)); //next time
				
				for(int j=0;predictions!=null&&j<predictions.length;j++){
					if(predictions[j].equalsIgnoreCase(event)){
						countOfCorrectPrediction++;
						break;
					}
				}
				//Add next event observation to the model
				appm.addEventObservation(currentTime,event, 0,0);
				//System.out.println("Observed Event: "+event+"");
				
				//Get Next Event predictions
				predictions = appm.getEventPredictions(0,0);
				countOfPrediction++;
				String buffer = "Predictions: ";
				for(String prediction:predictions)
					buffer += prediction+",";
				//System.out.println((i-1)+" "+buffer);
				//System.out.println();
				startTime = currentTime;
			}
			System.out.println("Get top "+TOPK+" predicted apps for next time point. Prediction Accuracy is "+
					(countOfCorrectPrediction*100/countOfPrediction)+"%.");
		}
	}
}
