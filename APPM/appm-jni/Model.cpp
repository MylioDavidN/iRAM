...
inline void PPM_CONTEXT::updateAccuracy(int symbol, int BUCKET)
{
    ...
    if(context) {
        p = context->Stats;
        BYTE* freqs = new BYTE[numChars+1];
        BYTE* symbols =  new BYTE[numChars+1];
        int i=numChars,j=0;
...
{
    double* nfreqArray = new double[len];
    ...
    while(numChars<=expectedChars && context!=NULL) {
        p = context->Stats;
        double w = weight;
        if(context->correct+context->incorrect!=0)
            w = (1.0*context->correct)/(context->correct+context->incorrect);
